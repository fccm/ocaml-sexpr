...
let parse pop_char =
  let rec aux st =
    ...
    | None ->
        ...
        | Parse_word _ ->
            failwith "Parsing error: word not closed by parenthesis"
        | Parse_string _ ->
            failwith "Parsing error: string content not closed by parenthesis"
        end
    | Some c ->
        match c, st with
        | ';', Parse_root _
...
let string_pop_char str =
  ...
  function () ->
    ...
    if !i >= len
    then None
    else Some (String.unsafe_get str !i)
...
let parse_from = function
  ...
  | `IC ic -> parse_ic ic
  | `File filename -> parse_file filename
